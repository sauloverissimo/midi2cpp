/*
 * main.cpp, arduino-nano-esp32-midi2-showcase
 *
 * Headless full-spec USB MIDI 2.0 device showcase on the Arduino Micro
 * ESP32 (ESP32-S3) native USB-OTG port. Mirrors the rp2040-midi2 22 s
 * scene cycle, exercising every category of message MIDI 2.0 brings
 * beyond MIDI 1.0. Each scene logs to UART and emits observable UMPs
 * on the native USB-OTG.
 *
 *   Boot (once):
 ...
 *     - On-board LED (LED_BUILTIN, GPIO48): lit while USB is mounted
 *
 * UART debug print at 115200 8N1 on the default ESP-IDF console UART.
 */
#include <cstdio>
#include <cstring>
#include <cmath>
...
static const char     kEndpointName[]  = "ArduinoNanoESP32";
static const char     kProductInstId[] = "ArduinoNanoESP32-showcase-0001";
...
static void install_ci_bootstrap(m2ci& ci) {
    ...
    rc = ci.addPropertyStatic("DeviceInfo",
        "{\"manufacturer\":\"github.com/sauloverissimo\","
         "\"family\":\"arduino-nano-esp32-midi2\","
         "\"model\":\"showcase\","
         "\"version\":\"0.1.0\"}");
...
extern "C" void app_main(void) {
    ...
    std::printf("\r\n===========================================\r\n");
    std::printf("  arduino-nano-esp32-midi2-showcase  (VID:PID 0xCAFE:0x4093)\r\n");
    std::printf("===========================================\r\n");

    static m2device midi;
    static m2ci     ci(midi);

    arduino_nano_esp32_midi2::init(midi, ci);
    midi.begin();
    ...
    while (true) {
        arduino_nano_esp32_midi2::task(midi);

        bool mounted = midi.isMounted();
        if (mounted != prev_mounted) {
            arduino_nano_esp32_midi2::led_show_mounted(mounted);
