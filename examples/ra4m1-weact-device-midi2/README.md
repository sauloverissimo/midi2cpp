...
MIDI-CI: Discovery + Profile Configuration (GM 1) + Property Exchange (DeviceInfo, ChannelList, ProgramList + built-in ResourceList) + Process Inquiry MIDI report, via the `m2ci` responder. Data-message coverage full-surface burst each cycle (every message category, scenes A-J compressed). Not covered: MIDI 1.0 emission (Alt 0 present in the descriptor for compatibility but unused).
