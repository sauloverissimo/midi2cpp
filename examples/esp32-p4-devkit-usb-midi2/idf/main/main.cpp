/*
 * main.cpp, esp32-p4-devkit-usb-midi2-showcase
 *
 * Headless full-spec USB MIDI 2.0 device showcase on the Waveshare
 * ESP32-P4-WIFI6-DEV-KIT native USB-OTG port. Mirrors the rp2040-midi2
 * 22 s scene cycle, exercising every category of message MIDI 2.0
 * brings beyond MIDI 1.0. The features are ordered to be inspectable:
 * each scene logs to UART (CH343 USB-to-UART bridge on the "ToUART"
 * USB-C jack) and emits observable UMPs on the native USB-OTG (the
 * "USB-Device" USB-C jack).
 *
 *   Boot (once):
 ...
 *
 * UART debug print on the CH343 USB-to-UART bridge ("ToUART" USB-C
 * jack) at 115200 8N1.
 */
#include <cstdio>
#include <cstring>
...
static const char     kEndpointName[]  = "ESP32P4DevKit";
static const char     kProductInstId[] = "ESP32P4DevKit-showcase-0001";
...
static void install_ci_bootstrap(m2ci& ci) {
    ...
    rc = ci.addPropertyStatic("DeviceInfo",
        "{\"manufacturer\":\"github.com/sauloverissimo\","
         "\"family\":\"esp32-p4-devkit-usb-midi2\","
         "\"model\":\"showcase\","
         "\"version\":\"0.1.0\"}");
...
extern "C" void app_main(void) {
    ...
    std::printf("\r\n===========================================\r\n");
    std::printf("  esp32-p4-devkit-usb-midi2-showcase  (VID:PID 0xCAFE:0x4091)\r\n");
    std::printf("===========================================\r\n");
