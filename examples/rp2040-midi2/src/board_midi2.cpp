...
// Drain immediately so the driver re-arms the OUT endpoint (no long NAK
// windows). Only MOVES packets into the ring; decode happens in task().
...
//
// stridx 4 points at the block name in string_desc_arr (usb_descriptors.c).
// It is not required for MIDI 2.0 conformance, but hosts that read only the
// static GTB descriptor, such as Windows MIDI Services, show an empty block
// name when it is 0. Hosts that query the UMP Stream responder, such as Linux,
// get the name from tud_midi2_fb_name_cb either way.
