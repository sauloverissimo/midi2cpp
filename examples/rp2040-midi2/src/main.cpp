/*
 * rp2040-midi2-showcase: full-surface USB MIDI 2.0 device showcase, ~22 s scene cycle
 * while mounted.
 *
 ...
 * (all in board_midi2.cpp). No app-side stream responder is installed;
 * Device Identity is carried by MIDI-CI Discovery (SysEx) via ci.begin.
 *--------------------------------------------------------------------*/
...
static void install_ci_bootstrap(m2ci& ci) {
    ...
    rc = ci.addPropertyStatic("DeviceInfo",
        "{\"manufacturerId\":[125,0,0],\"familyId\":[1,0],\"modelId\":[4,0],\"versionId\":[0,0,4,0],\"manufacturer\":\"midi2.diy\","
         "\"family\":\"RP2040\","
         "\"model\":\"RP2040 MIDI 2.0\","
         "\"version\":\"0.0.1\"}");
...
int main() {
    ...
    std::printf("\r\n===========================================\r\n");
    std::printf("  rp2040-midi2-showcase  (VID:PID 0xCAFE:0x4070)\r\n");
    std::printf("===========================================\r\n");
