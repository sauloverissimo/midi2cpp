/*
 * usb_descriptors.c, USB Device descriptors for t-display-s3-midi2.
 *
 * Identification:
 *   USB VID:PID         0xCAFE:0x4094   (TinyUSB educational VID + project PID)
 *   Manufacturer string midi2.diy
 *   Product string      TDisplayS3
 *
 * Configuration:
 *   1 MIDI 2.0 class interface (Audio Control + MIDI Streaming, Alt 0
 *   MIDI 1.0 fallback + Alt 1 UMP). Receiver-only recipe; the device
 *   does not emit notes by itself, it mirrors what the host sends on
 *   the on-board ST7789 piano roll.
 */
#include <string.h>
#include "tusb.h"
...
static char const *string_desc_arr[] = {
    ...
    "LILYGO T-Display S3 MIDI 2.0", /* 4: Group Terminal Block name      */
};
