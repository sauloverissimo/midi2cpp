...
static void scene_b_per_note(m2device& midi, Showcase& s, uint32_t t, uint32_t now) {
    if (t >= kB_NoteOnMs && t < kB_NoteOffMs + 500) {
        scene_display::set_scene(scene_display::Scene::B_PerNote, s.cycle_count);
        scene_display::notify_per_note(kB_Note,
            sinf((float)(t - kB_NoteOnMs) / 1000.0f * 2.0f * 3.14159265f * 5.0f));
    }
    ...
    if (s.b_on && !s.b_off && t < kB_NoteOffMs) {
        ...
        if (now - s.b_last_vib >= kB_VibUpdMs) {
            ...
            scene_display::bump_counter(scene_display::CounterKind::PitchBend);
            s.b_last_vib = now;
        }
    }
    ...
    if (s.b_on && !s.b_off && t >= kB_NoteOffMs) {
        ...
        scene_display::bump_counter(scene_display::CounterKind::NoteOff);
        s.b_off = true;
...
static void scene_c_walk(m2device& midi, Showcase& s, uint32_t t, uint32_t now) {
    if (t >= kC_StartMs && t < kD_Ms) {
        scene_display::set_scene(scene_display::Scene::C_Resolution, s.cycle_count);
    }
    if (s.c_idx < kC_Count && t >= kC_StartMs &&
        ...
        scene_display::notify_resolution(note, vel, cc_val, pb);
        scene_display::bump_counter(scene_display::CounterKind::NoteOn);
        scene_display::bump_counter(scene_display::CounterKind::CC);
        scene_display::bump_counter(scene_display::CounterKind::PitchBend);
...
static void scene_e_rpn_nrpn(m2device& midi, Showcase& s, uint32_t t) {
    if (t >= kE_RpnMs && t < kF_OnMs) {
        scene_display::set_scene(scene_display::Scene::E_RpnNrpn, s.cycle_count);
    }
    if (!s.e_rpn_done && t >= kE_RpnMs) {
        ...
        scene_display::notify_rpn(scene_display::RpnVariant::Rpn, 0, 0, 0x40000000);
    ...
    if (!s.e_nrpn_done && t >= kE_NrpnMs) {
        ...
        scene_display::notify_rpn(scene_display::RpnVariant::Nrpn, 0x12, 0x34,
                                  (int64_t)0xDEADBEEFu);
    ...
    if (!s.e_relrpn_done && t >= kE_RelRpnMs) {
        ...
        scene_display::notify_rpn(scene_display::RpnVariant::RelRpn, 0, 0, 0x01000000);
    ...
    if (!s.e_relnrpn_done && t >= kE_RelNrpnMs) {
        ...
        scene_display::notify_rpn(scene_display::RpnVariant::RelNrpn, 0x12, 0x34, -0x00800000);
...
static void scene_f_attribute(m2device& midi, Showcase& s, uint32_t t) {
    if (t >= kF_OnMs && t < kG_Ms) {
        scene_display::set_scene(scene_display::Scene::F_NoteAttribute, s.cycle_count);
        scene_display::notify_attribute(kF_Note, +50);
    }
    if (!s.f_on && t >= kF_OnMs) {
...
static void scene_g_sysex8(m2device& midi, Showcase& s, uint32_t t) {
    ...
    scene_display::notify_sysex8(payload, sizeof(payload));
    scene_display::bump_counter(scene_display::CounterKind::Other);
    std::printf("[G] SysEx8 emitted (%zu raw 8-bit bytes)\r\n", sizeof(payload));
    s.g_done = true;
}

static void scene_g_set(Showcase& s, uint32_t t) {
    if (t >= kG_Ms && t < kH_Ms) {
        scene_display::set_scene(scene_display::Scene::G_SysEx8, s.cycle_count);
    }
}

static void scene_h_dctpq(m2device& midi, Showcase& s, uint32_t t) {
    if (t >= kH_Ms && t < kI_Ms) {
        scene_display::set_scene(scene_display::Scene::H_DeltaClockstamp, s.cycle_count);
    }
    ...
    scene_display::notify_dctpq(480, 240);
...
static void scene_i_pe_notify(m2ci& ci, Showcase& s, uint32_t t) {
    if (t >= kI_Ms && t < kJ_Ms) {
        scene_display::set_scene(scene_display::Scene::I_PENotify, s.cycle_count);
    }
...
static void scene_j_clip_end(m2device& midi, Showcase& s, uint32_t t) {
    if (t >= kJ_Ms) {
        scene_display::set_scene(scene_display::Scene::J_EndOfClip, s.cycle_count);
    }
    if (s.j_done || t < kJ_Ms) return;
    midi.sendEndOfClip();
    scene_display::notify_end_of_clip();
...
static void showcase_step(m2device& midi, m2ci& ci, Showcase& s) {
    ...
    scene_display::set_progress((float)t / (float)kCycleMs);
}
...
int main() {
    ...
    // PWR_ON (GP22) gates the ST7789V panel rail. Drive HIGH before
    ...
    while (true) {
        ...
        if (mounted != last_mounted) {
            scene_display::set_mounted(mounted);
